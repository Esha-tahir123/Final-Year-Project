...
import { Trophy, Star, Medal, X ,Play, Send, SkipForward, Clock, CheckCircle } from "lucide-react";

const HTMLCSSAssessment = () => {
  ...
  useEffect(() => {
    const fetchTasks = () => {
      const shuffledTasks = htmlCssTasks
        .slice(0, 20)
  ...
  // Reset the timer when moving to the next task
  const resetTimer = () => {
    setTimeLeft(60);
  };
  ...
  // Run code in iframe for preview
  const handleRunCode = () => {
    if (iframeRef.current) {
      iframeRef.current.srcdoc = `
        <html>
          <head><style>${currentCode}</style></head>
  ...
  // Skip task and save with a score of 0
  ...
  const handleSubmitAnswer = async () => {
    ...
    try {
      payload = {
        ...
        language: currentTask.language || "HTML/CSS",
        task: {
  ...
  // Calculate total score and assign badge
  ...
  {/* Assessment completion view with improved UI */}
if (isCompleted) {
    ...
    {timeLeft <= 15 && (
      <span className="animate-pulse">
        ...
        <iframe
          ref={iframeRef}
          title="Code Output"
          className="w-full h-96 border mt-4 rounded-md"
        ></iframe>
      </div>
    </div>
    );
